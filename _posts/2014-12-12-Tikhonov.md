...
and the additional problem is not only how to calculate the singular values, but how different are the estimation $$x^{\delta}$$ and the true value $$x$$. However also there are a methodological problem on the definition of the solution given that is not true that allways $$y^{\delta}\in Im(K)$$. A new concept of solution is needed and it is usually formulated as an optimization rule:
